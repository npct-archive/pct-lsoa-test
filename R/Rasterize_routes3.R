...
matrix_master= data.table(gIntersects(lines, byid = T))
colnames(matrix_master) <- lines$id
matrix_master$id <- lines$id
setkey(matrix_master,id)
...
groups =  data.table(id=as.character(lines$id),nrow=as.integer(0))
...
for(i in 1:nrow(lines)){ #loop thought every line
  ...
  rowsum = data.table(name=matrix$id,count=rowSums(matrix[,!"id",with=FALSE])) 
  if(nrow(matrix) == 0){ 
    break()
  }
  else {
    ...
    partners1 = matrix[row1,!"id",with=FALSE]*1
    sel1 = partners1[,which(colSums(partners1)==0)]
    partners1 = partners1[,sel1,with=FALSE]
    partners_name1 = colnames(partners1)
    partners_name1 = c(partners_name1, "id")
    matrix = matrix[!(matrix$id %in% row1),!(colnames(matrix) %in% row1), with = FALSE]
    submatrix = matrix[(matrix$id %in% partners_name1),(colnames(matrix) %in% partners_name1), with = FALSE]
    #Inner Loop
    ...
    for(j in 1:nrow(submatrix)){
      if(nrow(submatrix) == 0){
        ...
        subrowsum = data.table(name=submatrix$id,count=rowSums(submatrix[,!"id",with=FALSE]))
        ...
        partners2 = matrix[row2,!"id",with=FALSE]*1
        sel2 = partners2[,which(colSums(partners2)==0)]
        partners2 = partners2[,sel2,with=FALSE]
        partners_name2 = colnames(partners2)
        
        matrix = matrix[!(matrix$id %in% row2),!(colnames(matrix) %in% row2), with = FALSE]
        submatrix = matrix[(submatrix$id %in% row2),(colnames(submatrix) %in% row2), with = FALSE]
        submatrix = matrix[(submatrix$id %in% partners_name2),(colnames(submatrix) %in% partners_name2), with = FALSE]
